...
export default function LoginPage() {
  ...
  const handle_submit = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (data.user.role === "PATIENT") window.location.href = "/patient/dashboard";
      else if (data.user.role === "DOCTOR") window.location.href = "/doctor/dashboard";
      else if (data.user.role === "HOSPITAL") window.location.href = "/hospital/dashboard";
      ...
      <div className="hidden lg:flex w-1/2 bg-[#1B2B6B] flex-col justify-between p-12">
        <div>
          <h1 className="text-white text-2xl font-bold tracking-tight">
            Kizito<span className="text-[#4DD9C0]">Health</span>
          </h1>
        </div>
        <div>
          ...
          <p className="text-blue-200 text-base">
            AI-powered symptom assessment and doctor connections — all in one place.
          </p>
        </div>
        <p className="text-blue-300 text-sm">© 2026 KizitoHealth. All rights reserved.</p>
      </div>
          ...
          <div className="space-y-5">
            ...
            <div>
              <div className="flex justify-between mb-1.5">
                ...
                <Link href="/forgot-password" className="text-sm text-[#1B2B6B] hover:underline">
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
            ...
            <p className="text-center text-sm text-gray-500">
              ...
              <Link href="/signup" className="text-[#1B2B6B] font-medium hover:underline">
                Sign up
              </Link>
            </p>
          </div>
